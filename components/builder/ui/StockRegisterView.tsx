...
import { saveProductAction, removeProductGridBlocksAction } from '@/app/actions/product';
...
const StockRegisterViewBase = ({ onBack, onRegister, isPageMode = false }: StockRegisterViewProperties) => {
  ...
  const gridButtons =[
    ...
    { label: "Gerenciar Produtos", icon: Trash2, action: 'openManagement', highlight: false, isManagement: true },
  ...
  const handleGridButtonClick = (action?: string) => {
    ...
    if (action === 'openManagement') setIsManagementOpen(true);
  ...
  const handleConfirmRegister = async () => {
    ...
    const hasVariationPhoto = savedVariations.some(variation => variation.images && variation.images.length > 0);
    ...
    try {
      ...
      if (directPhotoFile) {
        ...
        const itemWithPhoto = savedVariations.find(variation => variation.images && variation.images.length > 0);
      ...
      const result = await saveProductAction({
        ...
        variations: savedVariations.map(variation => ({ 
          ...variation, 
          name: sanitizedName,
          stock: variation.qty,
          qty: variation.qty
  ...
  return (
    <div className={cn(
      ...
      {/* 🧱 AJUSTE MILIMÉTRICO: pb-40 para garantir que o último item do grid não fique escondido atrás do footer */}
      <div className="flex-1 overflow-y-auto scrollbar-hide p-3 pb-40 space-y-2">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-2">
          {gridButtons.map((button, index) => {
            ...
            const hasVariations = button.action === 'openVariations' && savedVariations.length > 0;
            const hasPhoto = (button.action === 'openCamera' || button.action === 'openGallery') && directPhotoFile !== null;
            const isManagement = button.isManagement === true;
            const isActive = isPriceSet || hasVariations || hasPhoto;
            
            return (
              <button 
                key={index} 
                onClick={() => handleGridButtonClick(button.action)} 
                className={cn(
                  "flex flex-col items-center justify-center gap-1 py-2 px-1 bg-white border border-gray-200 rounded-xl shadow-sm active:scale-95 transition-all group", 
                  isActive && "border-[#00c853] bg-green-50/30", 
                  button.highlight && "border-orange-200 bg-orange-50/50", 
                  isManagement && "border-red-200 bg-red-50/30"
                )}
              >
                <div className={cn(
                  "w-10 h-10 rounded-full flex items-center justify-center", 
                  isActive ? "bg-[#00c853] text-white" : 
                  button.highlight ? "bg-orange-100 text-orange-600" : 
                  isManagement ? "bg-red-100 text-red-600" : "bg-gray-50 text-gray-600"
                )}>
                ...
                {isActive && (
                  <span className="text-[10px] font-bold text-[#00c853]">
                    {isPriceSet ? productPrice : `${savedVariations.length} item(s)`}
                  </span>
                )}
              </button>
            );
      ...
      {/* 🧱 AJUSTE MILIMÉTRICO: pb-[calc(1rem+env(safe-area-inset-bottom,16px))] para proteger o botão da barra do iOS */}
      <div className={cn(
        "absolute bottom-0 left-0 w-full p-3 pt-8 z-20",
        "pb-[calc(1rem+env(safe-area-inset-bottom,16px))]",
        "bg-gradient-to-t from-[#eeeeee] via-[#eeeeee] to-transparent", 
        isPageMode && "from-white via-white rounded-b-3xl"
      )}>
...
- **Safe Area**: Utiliza env(safe-area-inset-bottom) para evitar sobreposição com a Home Indicator do iOS.
    `,
    connectsTo:[
